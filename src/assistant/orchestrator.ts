...
import { getUserContext, formatContextForPrompt, type UserContext } from '../assistant/personalization';
...
export async function runSwarm(dilemmaText: string, taskList: Task[]): Promise<SwarmResult> {
  ...
  // Context Injection: built once per swarm run, before any agent is called,
  // then appended to every system prompt below. This is the single point
  // where personal context enters the reasoning flow — no agent calls the
  // LLM without it.
  const userContext = getUserContext();
  const contextBlock = formatContextForPrompt(userContext);
  const withContext = (systemPrompt: string) => `${systemPrompt}\n\n${contextBlock}`;

  const safeCall = async (prompt: string, msg: string, fallback: string): Promise<string> => {
    try {
      return await callAgent(withContext(prompt), msg);
    } catch (err) {
  ...
  let synthRaw: string;
  try {
    synthRaw = await callAgent(withContext(SYSTEM_PROMPTS.synthesizer), synthMessage);
  } catch (err) {
    console.error('Synthesizer API call failed, using fallback verdict:', err);
    return { positions, verdict: FALLBACK_VERDICT, userContext };
  }
  ...
  try {
    ...
    if (!verdict.contextRationale) {
      verdict.contextRationale = 'Synthesizer did not report a context rationale for this verdict.';
    }
  } catch (err) {
  ...
  return { positions, verdict, userContext };
}
...
export async function runEnforcer(
  ...
  const contextBlock = formatContextForPrompt(getUserContext());

  try {
    return await callAgent(`${filledPrompt}\n\n${contextBlock}`, `The user wants to override: "${overriddenPoint}". Their reason: "${userReason}"`);
  } catch (err) {
